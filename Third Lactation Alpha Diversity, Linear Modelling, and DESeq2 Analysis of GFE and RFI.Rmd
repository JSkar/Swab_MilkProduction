---
title: Third Lactation Alpha Diversity, Linear Modelling, and DESeq2 Analysis of GFE and RFI
...
#Load in the phyloseq object that contains
  ...
  #7000 sequences
  #3rd Lactation Arlington Animals

load("Third Lactation RFI and GFE Animals.RData")
...
RFI_List[cutoff]
  # -1.3
#Anything lower than -1.3 is considered HE

RFI_List[(length(RFI_List) - cutoff)]
  # 1
#Anything higher than 1 is considered LE

sample_data(physeq)$rfi_Efficiency <- ifelse(sample_data(physeq)$rfi_recent <= -1.3, "HE", ifelse(sample_data(physeq)$rfi_recent > 1, "LE", "MID"))
...
GFE_List[cutoff]
  # 1 
#Anything lower than 1  is considered LE

GFE_List[(length(GFE_List) - cutoff)]
  # 1.166667
#Anything higher than 1.166667 is considered HE

sample_data(physeq)$gfe_20percent <- ifelse(sample_data(physeq)$GFE <= 1 , "LE", ifelse(sample_data(physeq)$GFE >= 1.166667, "HE", "MID"))
...
p1.3 <- ggplot(sample_data(physeq.swab1.abund), aes(x=gfe_20percent, y=Shannon)) + 
  geom_boxplot() +
  labs(x="GFE Groups", y = "Shannon Diversity") +
  scale_x_discrete(limits = c("LE", "HE", "MID"))
p1.3

p2.3 <- ggplot(sample_data(physeq.swab1.abund), aes(x=gfe_20percent, y=Chao1)) + 
  geom_boxplot() +
  labs(x="GFE Groups", y = "Chao Richness") +
  scale_x_discrete(limits = c("LE", "HE", "MID")) 
p2.3

p3.3 <- ggplot(sample_data(physeq.swab1.abund), aes(x=gfe_20percent, y=InvSimpson)) + 
  geom_boxplot() +
...
p4.3 <- ggplot(sample_data(physeq.swab1.abund), aes(x=rfi_Efficiency, y=Shannon)) + 
  geom_boxplot() +
  labs(x="RFI Groups", y = "Shannon Diversity") +
  scale_x_discrete(limits = c("LE", "HE", "MID"))
p4.3

p5.3 <- ggplot(sample_data(physeq.swab1.abund), aes(x=rfi_Efficiency, y=Chao1)) + 
  geom_boxplot() +
  labs(x="RFI Groups", y = "Chao Richness") +
  scale_x_discrete(limits = c("LE", "HE", "MID")) 
p5.3

p6.3 <- ggplot(sample_data(physeq.swab1.abund), aes(x=rfi_Efficiency, y=InvSimpson)) + 
  geom_boxplot() +
  labs(x="RFI Groups", y = "Inverse Simpson") +
  scale_x_discrete(limits = c("LE", "HE", "MID")) 
p6.3

grid.arrange(p1.3, p2.3, p3.3, p4.3, p5.3, p6.3, ncol = 3, top="Alpha Diversities of Third Lactation")
...
ggplot(res_sig, aes(x=Genus, y=log2FoldChange, color=Phylum)) +
    ...
    ggtitle("DESeq2 Third Lactation GFE High vs Low")
...
ggplot2::ggplot(res_sig.rfi, aes(x=Genus, y=log2FoldChange, color=Phylum)) +
    geom_jitter(size=3, width = 0.2) +
    theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5)) +
    ggtitle("DESeq2 Third Lactation RFI High vs Low")
...
write.csv(DESeq_Results, file="DESeq2 Results Third Lact for GFE and RFI Groups.csv")
```


#Linear Modelling with CLR
```{r}
#Using the ggforce function "geom_mark_ellipse() to force draw an ellipse because there are too few points to draw ellipses (only 3 for each) using the normal method (stat_ellipse())

meta.third <- as.matrix(sample_data(physeq))
meta.third <- as.data.frame(meta.third)
...
z_third_rfi <- phyloseq::plot_ordination(physeq, ord_clr, type="samples", color="rfi_Efficiency") + 
  geom_point(size = 2) +
  coord_fixed() +
  xlim(-20, 20) +
  ylim(-20, 20) +
  ggtitle("Third Lactation PCA by RFI") +
  guides(color = guide_legend(title = "RFI Group")) +
  ggforce::geom_mark_ellipse(aes(color = rfi_Efficiency), fill = NA, linetype = "dashed") +
  scale_color_manual(values = okabe_ito_colors)

z_third_rfi

z_third_gfe <- phyloseq::plot_ordination(physeq, ord_clr, type="samples", color="gfe_20percent") + 
  geom_point(size = 2) +
  coord_fixed() +
  xlim(-20, 20) +
  ylim(-20, 20) +
  ggforce::geom_mark_ellipse(aes(color = gfe_20percent), fill = NA, linetype = "dashed") +
  ggtitle("Third Lactation PCA by GFE") +
...
meta.third.2 <- meta.third[order(row.names(meta.third)),]
...
meta.third.3 <- merge(meta.third.2, plot_data_2, by="row.names")
row.names(meta.third.3) <- meta.third.3$Row.names


meta.third.3$rfi_recent <- as.numeric(meta.third.3$rfi_recent)
meta.third.3$GFE <- as.numeric(meta.third.3$GFE)


##Linear modelling of GFE/RFI against PC1 axis for ALL ANIMALS
GFE.pc1.3 <- ggplot(meta.third.3, aes(GFE, PC1), geom_smooth(method="lm", se=FALSE)) +
    geom_point() +
    geom_smooth(method="lm", se=FALSE) +
    ggtitle("GFE x PC1")

GFE.pc2.3 <- ggplot(meta.third.3, aes(GFE, PC2), geom_smooth(method="lm", se=FALSE)) +
    geom_point() +
    geom_smooth(method="lm", se=FALSE) +
    ggtitle("GFE x PC2")
...
RFI.pc1.3 <- ggplot(meta.third.3, aes(rfi_recent, PC1)) +
    geom_point() +
    geom_smooth(method="lm", se=FALSE) +
    ggtitle("RFI x PC1")

RFI.pc2.3 <- ggplot(meta.third.3, aes(rfi_recent, PC2)) +
    geom_point() +
    xlab("RFI") +
    geom_smooth(method="lm", se=FALSE) +
    ggtitle("RFI x PC2")
...
lm_fit_GFE.PC1 <- lm(GFE ~ PC1, data=meta.third.3)
summary(lm_fit_GFE.PC1)
#Significant! p = 0.0451 

lm_fit_RFI.PC1 <- lm(rfi_recent ~ PC1, data=meta.third.3)
summary(lm_fit_RFI.PC1)
#Not significant



lm_fit_GFE.PC2 <- lm(GFE ~ PC2, data=meta.third.3)
summary(lm_fit_GFE.PC2)
#Not significant

lm_fit_RFI.PC2 <- lm(rfi_recent ~ PC2, data=meta.third.3)
